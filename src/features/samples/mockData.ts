...
export const initialSamples: SampleRequest[] = [
  ...
  return {
    id,campaignId,campaignName,sellerName,brandName,productName,optionName,quantity,requestedAt,requestedBy,managerId: userIdByName[String(managerName)] ?? `manager-${campaignId}`, managerName,orderManagerId: userIdByName[String(orderManagerName)] ?? `order-manager-${campaignId}`, orderManagerName,orderMethod,paymentType,costOwner,unitPrice: Number(sampleCost),sampleCost,shippingCost,orderStatus: status,deliveryStatus,status,returnRequired,returnDueDate: returnDueDate || undefined, settlementReflected, settlementAmount: paymentType === '유상' ? Number(sampleCost) * Number(quantity) + Number(shippingCost) : 0, memo: '', attachments: [], activityLogs: log('샘플 요청 생성'),
