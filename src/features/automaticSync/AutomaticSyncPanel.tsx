...
export function AutomaticSyncPanel() {
  ...
  async function excludeSelected() {
    ...
    try {
      await automaticSyncService.exclude(targets.map(change => change.id), exclusionReason)
      setSelected([]); setExcludeConfirm(false); setExclusionReason(''); setMessage(`${targets.length}건 등록 제외 완료 · 처리 이력이 보존됩니다.`)
