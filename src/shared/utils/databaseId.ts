export function toDatabaseUuid(value: string) {
  ...
  const multiplied = Math.imul(first, second) >>> 0
  const hex = `${(first >>> 0).toString(16).padStart(8, '0')}${(second >>> 0).toString(16).padStart(8, '0')}${((first ^ second) >>> 0).toString(16).padStart(8, '0')}${multiplied.toString(16).padStart(8, '0')}`
