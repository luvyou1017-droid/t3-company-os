...
export function truncateToWon(value: number) {
...
export function calculateWithholding(grossSettlementAmount: number, deductions = 0): WithholdingCalculation {
  ...
  const incomeTaxAmount = truncateToWon(incomeTaxRaw)
  const localIncomeTaxRaw = withholdingBaseAmount * 0.003
  const localIncomeTaxAmount = truncateToWon(localIncomeTaxRaw)
  ...
  return {
    ...
    log: [
      ...
      `소득세 3% = truncateToWon(${withholdingBaseAmount.toLocaleString('ko-KR')} × 0.03 = ${incomeTaxRaw}) = ${incomeTaxAmount.toLocaleString('ko-KR')}원`,
      `지방소득세 0.3% = truncateToWon(${withholdingBaseAmount.toLocaleString('ko-KR')} × 0.003 = ${localIncomeTaxRaw}) = ${localIncomeTaxAmount.toLocaleString('ko-KR')}원`,
...
export function runWithholdingAssertions() {
  ...
  const checks = {
    ...
    incomeTax: calculation.incomeTaxAmount === 14_539,
    localIncomeTax: calculation.localIncomeTaxAmount === 1_453,
    totalTax: calculation.totalWithholdingTaxAmount === 15_992,
    finalPayment: calculation.finalPaymentAmount === 468_663,
  }
