...
export function ProductListPage({ onOpen, permission }: { onOpen: (id?: string) => void; permission: ProductMasterPermission }) {
  ...
  const toggleActive = async (product: ProductMaster) => {
    const nextActive = !product.active
    if (!window.confirm(`${product.productName} 상품을 ${nextActive ? '사용 중' : '미사용'}으로 변경할까요?\n기존 공구·정산 이력은 보존됩니다.`)) return
    await productService.setProductActive(product.id, nextActive)
    await load()
  }
