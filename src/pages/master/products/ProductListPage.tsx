...
export function ProductListPage({ onOpen, permission }: { onOpen: (id?: string) => void; permission: ProductMasterPermission }) {
  ...
  const copySellerCatalogUrl = async () => {
    const url = `${window.location.origin}/seller/catalog`
    try { await navigator.clipboard.writeText(url); window.alert('카탈로그 주소를 복사했습니다.') }
    catch { window.prompt('아래 주소를 복사해주세요.', url) }
  }
  return <section className="master-page">
    <div className="master-page__heading"><div><p className="page-eyebrow">PRODUCT DATABASE</p><h1>공동구매 물품 DB</h1><p>제품과 SKU, 공개 범위, Campaign에 전달할 기본 조건을 관리합니다.</p></div><div className="button-row"><button className="secondary-button" onClick={() => void copySellerCatalogUrl()}>카탈로그 주소 복사</button><button className="secondary-button" onClick={openSellerCatalog}>셀러 화면 열기</button>{permission.canCreate && <button className="primary-button" onClick={() => onOpen()}>신규 제품 등록</button>}</div></div>
