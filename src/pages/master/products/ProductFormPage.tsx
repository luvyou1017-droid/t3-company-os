import { detailOptionCandidates } from '../../../features/productMaster/utils/detailOptionCandidates'
...
const initial: ProductMasterInput = {
  ...
  partnerPortalVisible: false, partnerDescription: '', partnerMinimumOrder: '', partnerSupplyNote: '',
  sellerPortalStatus: 'closed', badges: [], sampleAvailable: false, managerName: '김병희', managerContact: '',
  campaignReferences: [], lifecycleStatus: 'active', active: true, testData: false,
}
...
export function ProductFormPage({ productId, onBack, permission }: { productId?: string; onBack: () => void; permission: ProductMasterPermission }) {
  ...
  const rowsToSkus = (rows: Array<Record<string, unknown>>, mode: 'append' | 'names_only' | 'replace' = 'append') => {
    ...
    const next = usableRows.map((row, index): ProductSku => {
      ...
      const optionName = String(row['구성명'] ?? row['옵션명'] ?? '').trim()
      const skuCode = String(row['SKU코드'] ?? row['SKU 코드'] ?? '').trim()
      const identity = (value: string) => value.toLocaleLowerCase('ko-KR').replace(/[^0-9a-z가-힣]/g, '')
      const existing = form.skus.find((sku) => (skuCode && sku.skuCode === skuCode) || identity(sku.optionName) === identity(optionName))
      if (existing) matchedExistingIds.add(existing.id)
      return {
        ...existing,
        id: existing?.id ?? crypto.randomUUID(),
        skuCode: skuCode || existing?.skuCode || `SKU-${Date.now().toString().slice(-6)}-${index + 1}`,
        ...
        optionName,
        optionValues: String(row['세부옵션'] ?? '').trim() ? { 세부옵션: String(row['세부옵션']).trim() } : existing?.optionValues,
        ...
        sellerPortalVisible: existing?.sellerPortalVisible ?? true, representative: existing?.representative ?? (form.skus.length === 0 && index === 0), lifecycleStatus: existing?.lifecycleStatus ?? 'active', active: existing?.active ?? true, createdAt: existing?.createdAt ?? now, updatedAt: now,
      }
    })
    if (!next.length) { setSkuMessage('구성명이 있는 행을 찾지 못했습니다. 양식의 열 제목을 확인해주세요.'); return }
    const resultingSkus = mode === 'replace' ? [...next, ...form.skus.filter((sku) => !matchedExistingIds.has(sku.id))] : [...form.skus, ...next]
    ...
    setSkuMessage(`${next.length}개 구성을 반영했습니다. 기존 SKU ID와 파일에 없는 기존 SKU는 그대로 유지됩니다.`)
  }
  const pasteSkus = () => {
    const lines = skuPaste.trim().split(/\r?\n/).filter(Boolean)
    if (!lines.length) return
    const cells = lines.map((line) => line.split('\t'))
    const knownHeaders = ['NO', '카테고리', '상품명', '구성명', '옵션명', '세부옵션', '가격 적용 방식', '최소 수량', '최대 수량', '정상가', '공구판매가', '공구가', '총 매입가(VAT포함)', '총매입가', '공급가', '셀러 수수료율', '셀러수수료', '상태']
  ...
  const uploadWiseProposal = async (file?: File) => {
    ...
    try {
      ...
      patch('vendorName', parsed.metadata.vendorName || form.vendorName)
      patch('courierName', parsed.metadata.courierName || form.courierName)
      patch('shippingFee', parsed.metadata.shippingFee)
      patch('freeShippingThreshold', parsed.metadata.freeShippingThreshold)
      patch('jejuExtraFee', parsed.metadata.jejuExtraFee ?? form.jejuExtraFee)
      patch('islandExtraFee', parsed.metadata.islandExtraFee ?? form.islandExtraFee)
      patch('bundleShippingAvailable', parsed.metadata.bundleShippingAvailable ?? form.bundleShippingAvailable)
      patch('orderDeadlineTime', parsed.metadata.orderDeadlineTime || form.orderDeadlineTime)
      patch('manufactureInfo', parsed.metadata.manufactureInfo || form.manufactureInfo)
      patch('shelfLifeInfo', parsed.metadata.shelfLifeInfo || form.shelfLifeInfo)
      patch('sampleSupportType', parsed.metadata.sampleSupportType || form.sampleSupportType)
      patch('operatingInfo', { ...form.operatingInfo, courierName: parsed.metadata.courierName, baseShippingFee: parsed.metadata.shippingFee, freeShippingThreshold: parsed.metadata.freeShippingThreshold, jejuExtraFee: parsed.metadata.jejuExtraFee, islandExtraFee: parsed.metadata.islandExtraFee, returnShippingFee: parsed.metadata.returnShippingFee, exchangeShippingFee: parsed.metadata.exchangeShippingFee, bundleShippingAvailable: parsed.metadata.bundleShippingAvailable, orderDeadlineTime: parsed.metadata.orderDeadlineTime, stockInfo: parsed.metadata.stockInfo, manufactureInfo: parsed.metadata.manufactureInfo, shelfLifeInfo: parsed.metadata.shelfLifeInfo, linkProvided: parsed.metadata.linkProvided, sampleSupportType: parsed.metadata.sampleSupportType })
      patch('proposalSupplierInfo', { ...form.proposalSupplierInfo, vendorName: parsed.metadata.vendorName, businessRegistrationNumber: parsed.metadata.businessRegistrationNumber, address: parsed.metadata.vendorAddress, account: parsed.metadata.vendorAccount, taxInvoiceEmail: parsed.metadata.taxInvoiceEmail, orderContact: parsed.metadata.orderContact, orderEmail: parsed.metadata.orderEmail, csContact: parsed.metadata.csContact, csEmail: parsed.metadata.csEmail, settlementContact: parsed.metadata.settlementContact, settlementEmail: parsed.metadata.settlementEmail, mainBusinessEmail: parsed.metadata.mainBusinessEmail, businessHours: parsed.metadata.businessHours, salesHurdle: parsed.metadata.salesHurdle })
  ...
  return <section className="product-form-page">
    ...
    <section className="product-form-section" id="product-basic">{sectionTitle('1. 기본 정보', 'basic')}<div className="product-form-grid">
      ...
      <label className="product-field"><span>상품 상태</span><select value={form.lifecycleStatus ?? (form.active ? 'active' : 'inactive')} onChange={(event) => { const status = event.target.value as NonNullable<ProductMasterInput['lifecycleStatus']>; patch('lifecycleStatus', status); patch('active', status === 'active') }}><option value="active">사용</option><option value="inactive">미사용</option><option value="archived">삭제 보관</option></select></label>
    ...
    <section className="product-form-section"><h2>6. 협력 벤더 공개 정보</h2><p className="policy-note">승인된 협력 벤더에게만 공급가와 공급 조건이 표시됩니다. 회사 마진과 셀러 수수료는 공개하지 않습니다.</p><div className="product-form-grid">
      <BooleanSelect label="파트너 공급 상품" value={Boolean(form.partnerPortalVisible)} yesLabel="승인 벤더에게 공개" noLabel="벤더 비공개" onChange={(value) => patch('partnerPortalVisible', value)} />
      <label className="product-field"><span>최소 진행·발주 조건</span><input value={form.partnerMinimumOrder ?? ''} placeholder="예: 공구 매출 500만원 이상" onChange={(event) => patch('partnerMinimumOrder', event.target.value)} /></label>
      <label className="product-field product-span-2"><span>협력 벤더용 상품 설명</span><textarea value={form.partnerDescription ?? ''} onChange={(event) => patch('partnerDescription', event.target.value)} /></label>
      <label className="product-field product-span-2"><span>공급 조건 안내</span><textarea value={form.partnerSupplyNote ?? ''} placeholder="예: 일정·재고 확인 후 공급 확정" onChange={(event) => patch('partnerSupplyNote', event.target.value)} /></label>
    </div></section>
    <section className="product-form-section"><h2>7. 셀러 공개 정보</h2><p className="policy-note">이 영역만 셀러 카탈로그의 공개 전용 데이터로 변환됩니다. 매입가·회사 마진·PG·내부 메모는 전달되지 않습니다.</p><div className="product-form-grid">
    ...
    <section className="product-form-section" data-field="skus"><label><input type="checkbox" disabled={readOnly} checked={Boolean(form.sampleOnly)} onChange={event => patch('sampleOnly', event.target.checked)} />상품 전체 샘플 전용 · 해제 후 기존 SKU를 판매용으로 전환 가능</label><div className="sku-heading"><div><h2>2. 상품 구성표</h2><p>엑셀의 여러 행을 그대로 붙여넣거나 양식 파일을 올리면 한 번에 표로 등록됩니다.</p></div>{!readOnly && <button type="button" className="secondary-button" onClick={addSku}>한 줄 추가</button>}</div>
