...
export function ProductFormPage({ productId, onBack, permission }: { productId?: string; onBack: () => void; permission: ProductMasterPermission }) {
  ...
  const loadThumbnailFromProductUrl = async () => {
    ...
    try {
      ...
      const imageUrl = result.data?.image?.url?.trim()
      if (!imageUrl) throw new Error('thumbnail_not_found')
      patch('representativeImageUrl', imageUrl)
      patch('imageUrl', imageUrl)
      setThumbnailMessage('상품 썸네일을 불러왔습니다.')
    } catch {
      setThumbnailMessage('자동으로 불러오지 못했습니다. 아래 대표 이미지 URL 칸에 이미지 주소를 붙여넣어 주세요.')
    } finally {
  ...
  return <section className="product-form-page">
    ...
    <section className="product-form-section" id="product-basic">{sectionTitle('1. 기본 정보', 'basic')}<div className="product-form-grid">
      ...
      <label className="product-field product-span-2"><span>상품 페이지 URL</span><div className="button-row"><input type="url" value={productPageUrl} placeholder="상품 URL을 붙여넣으면 썸네일을 불러옵니다" onChange={(e) => setProductPageUrl(e.target.value)} onBlur={() => void loadThumbnailFromProductUrl()} /><button type="button" className="secondary-button" disabled={thumbnailLoading || !productPageUrl.trim()} onClick={() => void loadThumbnailFromProductUrl()}>{thumbnailLoading ? '불러오는 중' : '썸네일 불러오기'}</button></div>{thumbnailMessage && <small>{thumbnailMessage}</small>}</label>
