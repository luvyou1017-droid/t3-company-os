...
import { runSettlementAssertions, statusLabel, validateSettlement } from '../../shared/utils/settlement'
...
import { canEditSettlement, DEFAULT_OPERATOR_USER_ID, getUserById } from '../../shared/data/users'
...
export function SettlementDetailPage({ settlementId, onBack, onOpenSalesData }: { settlementId: string; onBack: () => void; onOpenSalesData?: (importId: string) => void }) {
  ...
  const [sellerRealNameDraft, setSellerRealNameDraft] = useState('')
  const [sellerResidentNumberDraft, setSellerResidentNumberDraft] = useState('')
  ...
  const campaign = getCampaign(settlement)
  const currentUser = getUserById(DEFAULT_OPERATOR_USER_ID)
  ...
  const sellerButtonBlockReasons = sellerPaymentRequest || settlement.sellerPaymentCompleted ? sellerRequestEditable ? [] : [paymentStateReason(sellerPaymentRequest, settlement.sellerPaymentCompleted)] : settlementConfirmed ? sellerRequestReasons : [confirmationReason]
  ...
  const saveSellerBusinessType = () => {
    if (!campaign) return
    setSellerSaveError('')
    const normalizedMasterType = normalizeSellerBusinessType(businessTypeDraft) ?? 'general_business'
    const isFreelancer = normalizedMasterType === 'freelancer'
    if (isFreelancer && !sellerRealNameDraft.trim()) { setSellerSaveError('실명을 입력해주세요.'); return }
    if (isFreelancer && !sensitiveIdentityService.has('seller', campaign.sellerId) && sellerResidentNumberDraft.replace(/[^0-9]/g, '').length !== 13) { setSellerSaveError('주민등록번호 13자리를 입력해주세요.'); return }
    if (!isFreelancer && !businessNameDraft.trim()) { setSellerSaveError('사업자명을 입력해주세요.'); return }
    try {
      if (isFreelancer && sellerResidentNumberDraft) sensitiveIdentityService.stage('seller', campaign.sellerId, sellerResidentNumberDraft)
      sellerMasterService.saveSellerProfile({ id: campaign.sellerId, name: campaign.sellerName, realName: isFreelancer ? sellerRealNameDraft.trim() : sellerProfile?.realName, businessType: normalizedMasterType, businessName: isFreelancer ? undefined : businessNameDraft.trim(), defaultMdId: campaign.mdId, defaultManagerId: campaign.managerId, bankName: sellerProfile?.bankName, accountNumber: sellerProfile?.accountNumber, accountHolder: sellerProfile?.accountHolder })
      const rule = sellerSettlementService.ensureSellerSettlementRule(settlement.campaignId)
      ...
      const latestProfile = sellerMasterService.getSellerById(campaign.sellerId)
      const latestRule = sellerSettlementService.getSellerSettlementRule(settlement.campaignId)
      if (normalizeSellerBusinessType(latestProfile?.businessType) !== normalizedMasterType || normalizeSellerBusinessType(latestRule?.businessType) !== normalizedMasterType) throw new Error('저장 결과를 확인하지 못했습니다.')
      ...
      setSellerSaveError('셀러 정보를 저장하지 못했습니다.')
      showClipboardToast('셀러 정보를 저장하지 못했습니다.', true)
    }
  }
  ...
  const saveSellerAccount = () => {
    ...
    sellerMasterService.saveSellerProfile({
      ...
      realName: sellerProfile?.realName,
  ...
  const openSellerBusinessType = () => {
    ...
    setSellerRealNameDraft(sellerProfile?.realName ?? (effectiveSellerBusinessType === 'freelancer' ? campaign?.sellerName ?? '' : ''))
    setSellerResidentNumberDraft('')
  ...
  const openSellerInfo = () => setReadinessModal('seller-info')
  ...
  return (
    <section className="settlement-detail-page">
        ...
        {readinessModal && campaign && <div className="settlement-modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setReadinessModal(null) }}><section aria-modal="true" className="settlement-readiness-modal" role="dialog"><button aria-label="닫기" className="settlement-expanded-close" onClick={() => setReadinessModal(null)} type="button">×</button>
          ...
          {readinessModal === 'business' && <><h2>셀러 정보 등록</h2><label className="form-field"><span>셀러명</span><input disabled value={campaign.sellerName} /></label><label className="form-field"><span>사업자 유형</span><select onChange={(event) => setBusinessTypeDraft(event.target.value as SellerBusinessType)} value={businessTypeDraft}><option value="general_business">법인/개인사업자</option><option value="simplified_business">간이사업자</option><option value="freelancer">개인 프리랜서</option></select></label>{businessTypeDraft === 'freelancer' ? <><label className="form-field"><span>실명</span><input autoComplete="name" onChange={(event) => setSellerRealNameDraft(event.target.value)} value={sellerRealNameDraft} /></label><label className="form-field"><span>주민등록번호</span><input autoComplete="off" inputMode="numeric" onChange={(event) => setSellerResidentNumberDraft(event.target.value)} placeholder="000000-0000000" type="password" value={sellerResidentNumberDraft} /></label><p>주민등록번호는 현재 로컬 저장소에 저장되지 않으며, 이 화면을 닫으면 폐기됩니다. 운영 연결 시 보안 서버에서 암호화·권한 기반으로 처리합니다.</p></> : <><label className="form-field"><span>사업자명</span><input onChange={(event) => setBusinessNameDraft(event.target.value)} placeholder="등록된 사업자명을 입력하세요" value={businessNameDraft} /></label><p>셀러명과 사업자명은 별도 정보로 저장됩니다. 증빙 유형은 사업자 유형에 따라 자동 결정됩니다.</p></>}{sellerSaveError && <p className="settlement-readiness-modal__error">{sellerSaveError}</p>}<div className="modal-actions"><button className="primary-button" disabled={businessTypeDraft === 'freelancer' ? !sellerRealNameDraft.trim() || (!sensitiveIdentityService.has('seller', campaign.sellerId) && sellerResidentNumberDraft.replace(/\D/g, '').length !== 13) : !businessNameDraft.trim()} onClick={saveSellerBusinessType} type="button">저장</button></div></>}
...
function SettlementRevisionModal({ campaign, currentUser, deductions, managerBusinessType, onClose, onSaved, rows, sellerBusinessType, settlement }: { campaign: Campaign; currentUser: AppUser; deductions: SettlementDeduction[]; managerBusinessType: SellerBusinessType; onClose: () => void; onSaved: (settlement: Settlement) => void; rows: SalesDataRow[]; sellerBusinessType?: SellerBusinessType; settlement: Settlement }) {
  const [draft, setDraft] = useState<SettlementRevisionDraft>(() => ({ settlementId: settlement.id, reason: settlement.sourceChangeReason || '정산 수정 요청 반영', rows: rows.map((row) => ({ ...row })), totalCommissionRate: settlement.currentCalculation.totalCommissionRate, sellerCommissionRate: settlement.currentCalculation.sellerCommissionRate, deductions: deductions.map((item) => ({ ...item })) }))
  ...
  const comparison = preview ? [
    ['총매출', settlement.currentCalculation.grossSales, preview.grossSales], ['이벤트비', settlement.currentCalculation.companyEventDeduction, preview.companyEventDeduction], ['최종 배분 대상 수수료', settlement.currentCalculation.distributableVendorCommission, preview.distributableVendorCommission], ['매니저 지급액', settlement.currentCalculation.managerAmount, preview.managerAmount], ['회사 귀속액', settlement.currentCalculation.companyAmount, preview.companyAmount], ['셀러 최종 입금액', settlement.currentCalculation.finalSellerPaymentAmount, preview.finalSellerPaymentAmount],
  ] as const : []
  ...
  return <div className="settlement-modal-backdrop"><section aria-modal="true" className="settlement-revision-modal" role="dialog"><button aria-label="닫기" className="settlement-expanded-close" onClick={onClose} type="button">×</button><header><div><span>v{settlement.settlementVersion} 수정</span><h2>정산서 수정하기</h2><p>현재 적용값을 기준으로 재계산하며, 저장 시 기존 버전은 보존됩니다.</p></div></header>
    <section><h3>기본 판매 데이터</h3><div className="table-scroll"><table className="data-table"><thead><tr><th>상품/SKU</th><th>판매수량</th><th>공구가</th><th>매출액</th></tr></thead><tbody>{draft.rows.map((row) => <tr key={row.id}><td>{campaign.productName} / {row.optionName}</td><td><input min="0" onChange={(event) => updateRow(row.id, { quantity: Number(event.target.value) })} type="number" value={row.quantity} /></td><td><input min="0" onChange={(event) => updateRow(row.id, { unitPrice: Number(event.target.value) })} type="number" value={row.unitPrice} /></td><td>{money(row.quantity * row.unitPrice)}</td></tr>)}</tbody></table></div></section>
    <section><h3>수수료</h3><div className="settlement-revision-grid"><label className="form-field"><span>총수수료율</span><input min="0" onChange={(event) => { setPreview(null); setDraft({ ...draft, totalCommissionRate: Number(event.target.value) }) }} type="number" value={draft.totalCommissionRate} /></label><label className="form-field"><span>셀러 수수료율</span><input min="0" onChange={(event) => { setPreview(null); setDraft({ ...draft, sellerCommissionRate: Number(event.target.value) }) }} type="number" value={draft.sellerCommissionRate} /></label><Summary label="총 판매 수수료" value={money((preview ?? settlement.currentCalculation).grossCommission)} /><Summary label="셀러 수수료" value={money((preview ?? settlement.currentCalculation).sellerCommissionAmount)} /></div></section>
    <section><h3>비용/차감</h3><div className="table-scroll"><table className="data-table"><thead><tr><th>항목</th><th>구분</th><th>금액</th><th>부담 주체</th></tr></thead><tbody>{draft.deductions.map((item) => <tr key={item.id}><td>{item.title}</td><td>{item.type === 'sample' ? '샘플비' : item.type === 'event' || item.type === 'promotion' ? '이벤트비' : item.type === 'purchase' ? '스룩페이/구매비' : '기타비용·차감'}</td><td><input min="0" onChange={(event) => updateDeduction(item.id, { amount: Number(event.target.value) })} type="number" value={item.amount} /></td><td><select onChange={(event) => updateDeduction(item.id, { costOwner: event.target.value as SettlementDeduction['costOwner'], applyLocation: event.target.value === 'company' ? 'net_company_commission' : event.target.value === 'seller' ? 'seller_payment' : event.target.value === 'manager' ? 'manager_payment' : 'record_only' })} value={item.costOwner}><option value="company">회사</option><option value="seller">셀러</option><option value="manager">매니저</option><option value="brand">업체</option></select></td></tr>)}</tbody></table></div></section>
    {events.length > 0 && <section><h3>연결 이벤트</h3><div className="settlement-event-summary-list">{events.map((event) => { const confirmedCost = campaignEventOperationService.getConfirmedSettlementCost(event); const actualCount = eventCounts[event.id] ?? 0; return <article key={event.id}><strong>{event.eventName}</strong><dl><div><dt>경품</dt><dd>{event.rewardProductName || '-'}</dd></div><div><dt>비용 처리</dt><dd>{event.costHandling === 'company_direct' ? '회사 직접 발송' : event.costHandling === 'vendor_free' ? '업체 무상 제공' : '매니저 선결제 · 승인형'}</dd></div><div><dt>예정</dt><dd>{event.plannedQuantity}명 / {money(event.estimatedTotalAmount)}</dd></div><div><dt>실제 확정</dt><dd>{event.winnerCountConfirmed ? `${event.confirmedQuantity ?? 0}명 / ${money(confirmedCost ?? 0)}` : '당첨자 인원 미확정'}</dd></div><div><dt>발송 상태</dt><dd>{event.shippingStatus}</dd></div></dl>{(!event.winnerCountConfirmed || event.plannedQuantity !== (event.confirmedQuantity ?? 0)) && <><p className="settlement-readiness-modal__error">이벤트 당첨자 등록 인원이 예정 인원과 다릅니다.</p><div className="event-confirm-actions"><input aria-label={`${event.eventName} 실제 당첨자 수`} min="0" onChange={(change) => setEventCounts({ ...eventCounts, [event.id]: Number(change.target.value) })} type="number" value={actualCount} /><button className="secondary-button" onClick={() => setEventCounts({ ...eventCounts, [event.id]: actualCount + 1 })} type="button">1명 추가 등록</button><button className="primary-button" onClick={() => confirmEvent(event)} type="button">{actualCount}명으로 이벤트 확정</button></div></>}</article> })}</div></section>}
    ...
    {preview && <section><h3>변경 전/후 비교</h3><table className="data-table settlement-revision-comparison"><thead><tr><th>항목</th><th>변경 전</th><th>변경 후</th></tr></thead><tbody>{comparison.map(([label, before, after]) => <tr className={before !== after ? 'is-changed' : ''} key={label}><th>{label}</th><td>{money(before)}</td><td>{money(after)}</td></tr>)}</tbody></table></section>}
    <label className="form-field"><span>수정 사유</span><textarea onChange={(event) => setDraft({ ...draft, reason: event.target.value })} rows={3} value={draft.reason} /></label>{error && <p className="settlement-readiness-modal__error">{error}</p>}<div className="modal-actions"><button className="secondary-button" onClick={onClose} type="button">취소</button><button className="secondary-button" onClick={calculate} type="button">변경 후 다시 계산</button><button className="primary-button" disabled={!preview || !draft.reason.trim()} onClick={save} type="button">수정 저장</button></div></section></div>
}
...
function ManagerSettlementDocument({ documentRef, exportGeneratedAt, rows, settlement }: { documentRef: RefObject<HTMLDivElement | null>; exportGeneratedAt: string; rows: SalesDataRow[]; settlement: Settlement }) {
  ...
  const managerProductRows = rows.map((row) => {
    ...
    return { row, totalRate, productAmount, productName, salesAmount: row.unitPrice * productAmount.quantity, supplyAmount: productAmount.supplyPrice * productAmount.quantity }
  })
...
function SellerSettlementDocument({ exportGeneratedAt, rows, sellerDocumentRef, settlement }: { exportGeneratedAt: string; rows: SalesDataRow[]; sellerDocumentRef: RefObject<HTMLDivElement | null>; settlement: Settlement }) {
  ...
  const evidenceName = sellerBusinessType === 'freelancer' ? '원천세 리스트 등록' : sellerBusinessType === 'simplified_business' ? '현금영수증 발행' : sellerBusinessType ? '세금계산서 발행' : '데이터 미연결'
...
function PaymentRequestStatusModal({ accountConfirmed, campaign, completed, onClose, ownerType, request, settlement }: {
  ...
  const businessType = request?.businessType ?? (isSeller ? sellerSettlementService.getSellerSettlementRule(settlement.campaignId)?.businessType : managerPaymentService.getBusinessType(campaign.managerName)) ?? 'general_business'
...
function PaymentRequestEvidenceModal({ actorName, campaign, existingRequest, managerBusinessType, onCanceled, onClose, onFailed, onRequested, ownerType, sellerBusinessType, settlement }: {
  ...
  const submit = async () => {
    ...
    setError('')
    try {
      if (!accountConfirmed) throw new Error('은행명, 계좌번호, 예금주명을 모두 입력해주세요.')
      if (!existingRequest && !isFreelancer && !file) throw new Error(`${evidenceName} 파일을 첨부해주세요.`)
      ...
      if (existingRequest) {
        paymentRequestService.updatePaymentRequest(existingRequest.id, { memo: memo.trim(), accountConfirmed: true, evidenceStatus: file ? 'pending' : existingRequest.evidenceStatus })
        if (file) paymentEvidenceService.linkToPaymentRequest(settlement.id, ownerType, existingRequest.id)
        onRequested('지급요청이 수정되었습니다.')
      } else {
        if (isSeller) paymentRequestService.createPaymentRequest(settlement.id, '허수정', { allowEvidencePending, memo, accountConfirmed })
        else paymentRequestService.createManagerPaymentRequest(settlement.id, '허수정', businessType, undefined, { allowEvidencePending, memo, accountConfirmed })
        onRequested(isFreelancer ? '원천세 리스트 등록 및 지급 요청이 완료되었습니다.' : '지급 요청이 완료되었습니다.')
      }
    } catch (submitError) {
  ...
  return <div className="settlement-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section aria-labelledby="payment-request-modal-title" aria-modal="true" className="settlement-modal payment-request-modal" role="dialog">
      ...
      <fieldset className="payment-account-fields"><legend>지급계좌</legend><label className="form-field"><span>은행명</span><input onChange={(event) => setPaymentAccountDraft((value) => ({ ...value, bankName: event.target.value }))} value={paymentAccountDraft.bankName} /></label><label className="form-field"><span>계좌번호</span><input onChange={(event) => setPaymentAccountDraft((value) => ({ ...value, accountNumber: event.target.value }))} value={paymentAccountDraft.accountNumber} /></label><label className="form-field"><span>예금주명</span><input onChange={(event) => setPaymentAccountDraft((value) => ({ ...value, accountHolder: event.target.value }))} value={paymentAccountDraft.accountHolder} /></label></fieldset>
