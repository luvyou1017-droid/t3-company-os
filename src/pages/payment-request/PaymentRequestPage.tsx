import { AccountingManagement } from '../settlement/components/AccountingManagement'
...
export function PaymentRequestPage() {
  ...
  return <><nav className="settlement-view-tabs" aria-label="지급승인 화면 선택">
    <button type="button" aria-pressed={tab !== 'withholding'} onClick={() => setTabState('requests')}>지급승인</button>
    <button type="button" aria-pressed={tab === 'withholding'} onClick={() => setTabState('withholding')}>원천세 관리 · 월별 마감</button>
  </nav>{tab === 'withholding' ? <AccountingManagement /> : <PaymentTransferWorkspace evidence={evidence} onSync={sync} requests={requests} />}</>
}
...
function PaymentTransferWorkspace({ requests, evidence, onSync }: { requests: PaymentRequest[]; evidence: PaymentEvidence[]; onSync: () => void }) {
  ...
  const calculateCurrentAmount = (request: PaymentRequest) => {
    if (request.recipientType === 'seller') {
      try { return sellerSettlementService.createSellerDocument(request.settlementId, false, request.businessType).calculation.finalSellerPaymentAmount }
...
function PaymentWorkflowDetail({ target, backLabel, onBack, onSync }: { target: WorkflowTarget; backLabel: string; onBack: () => void; onSync: () => void }) {
  ...
  const freelancerCalculation = taxItem ?? (isSeller ? sellerSettlementService.createSellerDocument(settlement.id, false, businessType).calculation.withholding : undefined) ?? calculateWithholding(grossSettlementAmount(settlement, target.recipientType), isSeller ? settlement.currentCalculation.sellerDeductionTotal : settlement.currentCalculation.managerDeductionTotal)
  return <section className="payment-workflow-detail">
...
function WithholdingTab({ assertion, onSync }: { assertion: ReturnType<typeof runWithholdingAssertions>; onSync: () => void }) {
  ...
  return <>
    ...
    <section className="workspace-card">
      ...
      <div className="responsive-table tax-table"><table><thead><tr><th>지급월</th><th>지급일</th><th>공동구매</th><th>구분</th><th>셀러 또는 매니저</th><th>부가세 포함 정산금</th><th>부가세 제외 기준금액</th><th>소득세 3%</th><th>지방소득세 (소득세의 10%)</th><th>총 원천징수액</th><th>최종 지급액</th><th>상태</th><th>담당자</th><th>수동 처리</th></tr></thead>
